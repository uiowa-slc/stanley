...
<div class="exhibit-largeimg" style="background-image: url($EventPageImage.URL);">
	...
	<div class="inner">
		<div class="container">
			<% if $StartDate || $EventLocation %><h3 class="exhibitinner-date"><% if $StartDate %>$StartDate.Format('F d')<% end_if %><% if $EndDate %> - $EndDate.Format('F d') <% end_if %><% if $EventLocation %> | $EventLocation<% end_if %></h3><% end_if %>
